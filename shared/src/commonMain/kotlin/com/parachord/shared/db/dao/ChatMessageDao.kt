...
class ChatMessageDao(private val db: ParachordDb, private val driver: SqlDriver) {
    ...
    suspend fun getByProvider(providerId: String): List<ChatMessageRecord> = withContext(Dispatchers.Default) {
    ...
    suspend fun insert(entity: ChatMessageRecord): Long = withContext(Dispatchers.Default) {
        queries.insert(
    ...
    suspend fun insertAll(entities: List<ChatMessageRecord>): Unit = withContext(Dispatchers.Default) {
        queries.transaction {
    ...
    suspend fun clearByProvider(providerId: String): Unit = withContext(Dispatchers.Default) {
    ...
    suspend fun deleteOlderThan(cutoffMillis: Long): Unit = withContext(Dispatchers.Default) {
        queries.deleteOlderThan(cutoffMillis)
    }

    /** Keep only the most recent [limit] messages per provider (trim old ones). */
    suspend fun trimToLimit(providerId: String, limit: Int): Unit = withContext(Dispatchers.Default) {
        queries.trimToLimit(
            providerId = providerId,
