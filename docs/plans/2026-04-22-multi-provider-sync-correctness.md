...
1. **Apple Music sync scope for v1.** Full parity with desktop in the initial release (tracks, albums, artists, playlists pull + push + the Apple-specific endpoint degradations) — or limited subset (e.g. playlists only, no library push) to ship faster?
2. **Schema approach.** Option A (JSON columns on `playlists`) or Option B (relational: extend `sync_playlist_link`, new `sync_playlist_source` table)? Recommendation: **B**.
3. **`localOnly` opt-out.** Port from desktop now, or defer? Without it, users can't say "this playlist never leaves my phone." Recommendation: **port**, trivial once the schema has room.
4. **`pendingAction` per-provider.** Implement now (mark remote-deleted locals, surface to UI) or defer to a follow-up? Recommendation: **defer**, surface as a toast on detection for v1.
5. **Removal of legacy `playlists.spotifyId` scalar.** Keep it as a Spotify-only convenience cache (current approach) or deprecate once `sync_playlist_link` carries the same info? Recommendation: **keep** for now — UI / share code reads from it.
6. **Reconnect-after-gap snapshot comparison.** Per `ProviderFeatures.snapshots` — Spotify uses `Opaque`, Apple uses `DateString`, future providers declare their own. Is the per-provider declaration sufficient, or do we also want a local content hash as a universal fallback (covering `SnapshotKind.None` providers without forcing always-pull)? Recommendation: **declared semantics only for v1**, content hash later if a `None`-snapshot provider ever joins.
7. **Cross-device locking.** Confirmed out of scope — Spotify is the merge oracle. Sanity check.
8. **UI for "Apple Music deletion unsupported".** Toast, banner, or silent-except-in-logs? Recommendation: **toast** when user explicitly triggers delete; silent otherwise.
